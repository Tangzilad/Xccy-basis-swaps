...
def render_page() -> None:
    ...
    render_calculation_windows([
        CalculationWindow(
            ...
            meaning="Direct domestic funding rate after extra spread adjustments.",
            significance="Baseline comparator for synthetic funding alternatives.",
            formula=r"r_{dom}=r_{domcurve}+s_{extra}",
            methodology="Add implementation spread to underlying domestic curve rate.",
            inputs="Domestic curve rate and extra spread in annualized percent.",
        ...
        CalculationWindow(
            ...
            meaning="Synthetic domestic funding from foreign curve plus basis and spread.",
            significance="Shows the executable synthetic route cost.",
            formula=r"r_{syn}=r_{forcurve}+b+s_{extra}",
            methodology="Translate foreign funding route into domestic all-in cost.",
            inputs="Foreign curve rate, basis, and extra spread.",
        ...
        CalculationWindow(
            ...
            meaning="Relative economics of synthetic versus domestic funding.",
            significance="Primary decision metric for route preference.",
            formula=r"\Delta r=r_{syn}-r_{dom}",
            methodology="Subtract direct domestic cost from synthetic all-in cost.",
            inputs="Synthetic and domestic all-in annualized rates.",
    ...
    render_calculation_windows(
        [
            CalculationWindow(
                title="Domestic all-in",
                meaning="All-in domestic funding rate after extra spread adjustments.",
                significance="Baseline comparator for synthetic funding alternatives.",
                formula=r"r_{dom}=r_{domcurve}+s_{extra}",
                methodology="Add implementation spread to direct domestic curve rate.",
                inputs="Domestic curve rate and extra spread.",
                substituted_values=f"$r_{{domcurve}}={(one['HUF direct'] - one['extra_spread']):.4%}, s_{{extra}}={one['extra_spread']:.4%}$",
                derivation_steps=("Take domestic curve rate.", "Add extra spread.",),
                assumptions=("Costs add positively.",),
                interpretation="Higher value means more expensive direct domestic funding.",
                common_misunderstandings=("Comparing domestic all-in against unadjusted synthetic leg.",),
                result=f"{one['HUF direct']:.4%}",
            ),
            CalculationWindow(
                title="Synthetic HUF all-in",
                meaning="Synthetic domestic funding from foreign curve plus basis and spread.",
                significance="Shows executable synthetic route cost.",
                formula=r"r_{syn}=r_{forcurve}+b+s_{extra}",
                methodology="Translate foreign funding route into domestic all-in cost.",
                inputs="Foreign curve rate, basis, and extra spread.",
                substituted_values=f"$r_{{forcurve}}={(one['USD direct'] - one['extra_spread']):.4%}, b={one['basis']:.4%}$",
                derivation_steps=("Start from foreign curve rate.", "Add basis.", "Add extra spread.",),
                assumptions=("Positive basis raises synthetic cost.",),
                interpretation="Higher value means synthetic domestic funding is more expensive.",
                common_misunderstandings=("Ignoring sign convention on basis when comparing routes.",),
                result=f"{one['HUF synthetic']:.4%}",
            ),
            CalculationWindow(
                title="Cross-market gap",
                meaning="Relative economics of synthetic versus domestic funding.",
                significance="Decision metric for route preference.",
                formula=r"\Delta r=r_{syn}-r_{dom}",
                methodology="Subtract direct domestic cost from synthetic all-in cost.",
                inputs="Synthetic and domestic all-in rates.",
                substituted_values=f"${one['HUF synthetic']:.6f}-{one['HUF direct']:.6f}$",
                derivation_steps=("Compute synthetic all-in.", "Compute domestic all-in.", "Take difference.",),
                assumptions=("Positive gap: synthetic is more expensive. Negative gap: synthetic saves money.",),
                interpretation="Positive gap penalizes synthetic route; negative favors synthetic route.",
                common_misunderstandings=("Using absolute level instead of relative gap for route choice.",),
                result=f"{one['HUF delta'] * 10000:.2f} bps",
            ),
            CalculationWindow(
                title="HUF issuance choice",
                meaning="Choice metric for issuing in HUF directly vs swapped alternative.",
                significance="Converts spread math into actionable issuance recommendation.",
                formula=r"\Delta_{dom}=r_{syn,dom}-r_{dir,dom}",
                methodology="Compare synthetic-domestic and direct-domestic all-in rates.",
                inputs="Synthetic and direct domestic all-in rates.",
                substituted_values=f"${one['HUF synthetic']:.6f}-{one['HUF direct']:.6f}$",
                derivation_steps=("Compute delta between routes.", "Select cheaper route.",),
                assumptions=("Negative delta means swapped issuance is cheaper.",),
                interpretation="Reported preference indicates lower-cost issuance channel.",
                common_misunderstandings=("Ignoring sign of delta when selecting preferred route.",),
                result=f"{huf_choice.preferred_route}; savings {huf_choice.savings_bp:.2f} bps",
            ),
            CalculationWindow(
                title="USD issuance choice",
                meaning="Choice metric for issuing in USD directly vs swapped alternative.",
                significance="Highlights cross-market relative value from USD perspective.",
                formula=r"\Delta_{for}=r_{syn,for}-r_{dir,for}",
                methodology="Compare synthetic-foreign and direct-foreign all-in rates.",
                inputs="Synthetic and direct USD all-in rates.",
                substituted_values=f"${one['USD synthetic']:.6f}-{one['USD direct']:.6f}$",
                derivation_steps=("Compute USD-route delta.", "Select cheaper route.",),
                assumptions=("Negative delta means swapped issuance is cheaper.",),
                interpretation="Preference reflects lower all-in USD issuance economics.",
                common_misunderstandings=("Confusing domestic-route sign convention with USD-route delta.",),
