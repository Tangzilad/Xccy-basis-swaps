from __future__ import annotations

import streamlit as st

from shared_page_helpers import render_page_footer, render_page_header
from src.analytics.conversion_factor import (
    conversion_factor_curve_aware,
    conversion_factor_simple,
    spread_translation_round_trip_bp,
    translate_spread_bp,
)
from src.analytics.hedging import (
    hedged_pickup_decomposition_bp,
    matched_vs_rolling_hedge_economics_bp,
    roll_cost_and_risk_proxy_bp,
)
from src.state.session_access import get_canonical_market_context
from streamlit_calc_helpers import CalculationWindow, render_calculation_windows
from ui_shell import learning_hint
from src.explainers.theory_panels import render_pedagogical_scaffold

REQUIRED_CALCULATION_WINDOWS: tuple[str, ...] = (
    "conversion_factor",
    "relative_forward_difference",
    "theoretical_forward",
    "hedged_pickup",
    "implied_usd_rate",
)


def _build_payload(session_state: dict) -> dict[str, object]:
    context = get_canonical_market_context(session_state)
    base_summary = context["summary_1y"]["base"]

    spot = float(base_summary["spot_fx"])
    usd = float(base_summary["usd_rate"])
    huf = float(base_summary["huf_rate"])
    basis = float(base_summary["basis_bps"])
    fwd_1y = spot * (1 + huf) / (1 + usd)

    simple_cf_payload = conversion_factor_simple(spot, fwd_1y)
    simple_cf = float(simple_cf_payload["conversion_factor"])

    tenor_years = [0.5, 1.0, 2.0]
    forward_curve = [spot * (1 + huf * tenor) / (1 + usd * tenor) for tenor in tenor_years]
    discount_factors = [1.0 / (1.0 + usd * tenor) for tenor in tenor_years]
    curve_cf_payload = conversion_factor_curve_aware(
        spot_huf_per_usd=spot,
        forward_huf_per_usd_by_tenor=forward_curve,
        tenor_years=tenor_years,
        discount_factors=discount_factors,
    )
    curve_cf = float(curve_cf_payload["conversion_factor"])

    # --- Pickup calculations ---
    nominal_diff_bp = (huf - usd) * 10_000
    gross = translate_spread_bp(nominal_diff_bp, curve_cf)
    round_trip = spread_translation_round_trip_bp(nominal_diff_bp, curve_cf, tolerance_bp=1e-6)

    rows = []
    for hc in [20.0, 35.0, 50.0]:
        rp = roll_cost_and_risk_proxy_bp(hc, hc + 5.0, 18.0, 1.0)
        ch = matched_vs_rolling_hedge_economics_bp(hc + 12.0, hc, rp["roll_risk_proxy_bp"], 0.6)
        decomp = hedged_pickup_decomposition_bp(gross, hc, abs(basis), 8.0)
        rows.append({"hedge_cost": hc, "pickup": decomp["net_hedged_pickup_bp"], **decomp, **rp, **ch})
    base = next(r for r in rows if r["hedge_cost"] == 35.0)

    # --- Header with learning objectives ---
    render_page_header(5, "6. Hedged Pickup and Hedge Choice")

    # --- Metrics ---
    a, b, c, d = st.columns(4)
    a.metric("Simple CF (F/S)", f"{simple_cf:.6f}")
    b.metric("Curve-aware CF", f"{curve_cf:.6f}")
    c.metric("Converted gross", f"{gross:.2f} bps")
    d.metric("Net pickup", f"{base['pickup']:.2f} bps")
    st.caption(f"Preferred hedge method (base case): **{str(base['preferred_hedge']).title()}**")

    # --- Charts ---
    st.line_chart(
        {
            "hedge_cost": [r["hedge_cost"] for r in rows],
            "pickup": [r["pickup"] for r in rows],
            "benefit_of_rolling": [r["benefit_of_rolling_bp"] for r in rows],
        },
        x="hedge_cost",
    )
    st.dataframe(rows, use_container_width=True)

    # --- Explanatory content ---
    st.markdown(
        """
**Why CF is not "just FX":** the conversion factor is the map from one quote space (HUF-bp) into another
(USD-bp). Spot alone does not control that map -- forward points and curve weights matter, which is why the
curve-aware CF can differ from the simple (F/S) ratio.

**Why hedged pickup differs from nominal yield differential:** the nominal HUF-USD differential is only a
starting point. Realized hedged pickup subtracts hedge implementation cost, basis drag, and frictions.
Therefore "higher nominal yield" can still produce weak or negative net pickup after hedging.
"""
    )

    learning_hint(
        "Compare the curve-aware CF against the simple CF. When do they diverge materially? "
        "This matters because the wrong conversion factor can make you think a trade is profitable "
        "when it is not. Rolling hedges introduce mark-to-market risk scaled by volatility."
    )

    # --- Calculation windows ---
    render_calculation_windows(
        [
                CalculationWindow(
                    title="Simple conversion factor",
                    meaning="Tenor-matched forward/spot ratio for quote-space conversion.",
                    significance="Provides a quick mapping from HUF-bp to USD-bp space.",
                    formula=r"CF_{simple}=F/S",
                    methodology="Use tenor-matched forward divided by spot.",
                    inputs=f"1Y forward F={fwd_1y:.4f} and spot S={spot:.4f}.",
                    substituted_values=f"$F={fwd_1y:.4f}, S={spot:.4f}$",
                    derivation_steps=("Collect matched-tenor forward and spot.", "Compute F/S.",),
                    assumptions=("Simple tenor-matched ratio for quote translation.",),
                    interpretation="Higher CF amplifies translated spread in USD-bp terms.",
                    common_misunderstandings=("Using unmatched tenors for forward and spot ratio.",),
                    result=f"{simple_cf:.6f}",
                ),
            CalculationWindow(
                title="Curve-aware conversion factor",
                meaning="Weighted conversion factor across tenor buckets.",
                significance="Improves translation accuracy when carry spans the curve.",
                formula=r"CF_{curve}=\sum_i w_i(F_i/S)",
                methodology="Apply annuity-style weights to tenor-specific forward ratios.",
                inputs=f"Spot S={spot:.4f}, tenor forwards, and normalized weights.",
                substituted_values=f"$\\sum_i w_i=1, S={spot:.4f}$",
                derivation_steps=("Compute each bucket F_i/S.", "Weight by w_i.", "Sum weighted ratios.",),
                assumptions=("Weights use discount-factor annuity terms across tenor buckets.",),
                interpretation="Curve-aware CF stabilizes translation when exposure spans maturities.",
                common_misunderstandings=("Treating single-tenor CF as universally accurate.",),
                result=f"{curve_cf:.6f}",
            ),
            CalculationWindow(
                title="Translated gross pickup",
                meaning="Nominal differential translated into comparison space using CF.",
                significance="Intermediate gross carry before hedge and friction deductions.",
                formula=r"\text{gross}_{tr}=\text{nominal diff}\times CF",
                methodology="Multiply nominal differential by selected conversion factor.",
                inputs="Nominal differential (bp) and conversion factor.",
                substituted_values=f"$\\text{{nominal diff}}={nominal_diff_bp:.2f}, CF={curve_cf:.6f}$",
                derivation_steps=("Measure nominal differential.", "Apply CF scaling.",),
                assumptions=("Positive is favorable before costs.",),
                interpretation="Positive gross value indicates preliminary carry advantage.",
                common_misunderstandings=("Treating gross translated pickup as net executable pickup.",),
                result=f"{gross:.2f} bps",
            ),
            CalculationWindow(
                title="Spread translation round-trip",
                meaning="Consistency check for conversion-factor mapping.",
                significance="Validates numerical stability and unit discipline.",
                formula=r"\text{HUF bp}\to\text{USD bp}\to\text{HUF bp}",
                methodology="Translate spread out-and-back and measure residual.",
                inputs="Input spread, translated spread, and reverse translation.",
                substituted_values=f"${round_trip['huf_bp_in']:.2f}\\to{round_trip['usd_bp_translated']:.2f}\\to{round_trip['huf_bp_round_trip']:.2f}$",
                derivation_steps=("Translate HUF to USD space.", "Translate back to HUF.", "Compute residual.",),
                assumptions=("Residual near zero validates conversion consistency.",),
                interpretation="Small residual indicates coherent conversion mechanics.",
                common_misunderstandings=("Ignoring round-trip residual when validating translation.",),
                result=f"residual={round_trip['round_trip_residual_bp']:.6f} bp | pass={round_trip['round_trip_within_tolerance']}",
            ),
            CalculationWindow(
                title="Pickup decomposition",
                meaning="Net hedged pickup decomposition from gross carry to executable edge.",
                significance="Shows how nominal carry is consumed by implementation drags.",
                formula=r"\text{Net}=\text{Gross}-\text{Hedge}-\text{Basis drag}-\text{Extra friction}",
                methodology="Subtract hedge, basis drag, and extra friction from gross pickup.",
                inputs="Gross pickup, hedge cost, basis drag, and extra friction (bps).",
                substituted_values=f"${base['gross_pickup_bp']:.2f}-{base['hedge_cost_bp']:.2f}-{base['basis_drag_bp']:.2f}-{base['extra_friction_bp']:.2f}$",
                derivation_steps=("Start from gross pickup.", "Subtract each drag component.",),
                assumptions=("Gross pickup is reduced by implementable hedge and friction terms.",),
                interpretation="Positive net pickup indicates remaining executable carry.",
                common_misunderstandings=("Equating gross pickup with net tradable pickup.",),
                result=f"{base['net_hedged_pickup_bp']:.2f} bps",
            ),
            CalculationWindow(
                title="Matched vs rolling hedge",
                meaning="Risk-adjusted comparison of hedge implementation choices.",
                significance="Determines preferred hedge structure beyond expected carry alone.",
                formula=r"\text{RA roll}=C_{roll}+\lambda\cdot \sigma_{roll}",
                methodology="Compare matched cost against risk-adjusted rolling cost.",
                inputs="Matched cost, rolling cost, roll-risk proxy, and risk-aversion multiplier.",
                substituted_values=(
                    f"$C_m={base['matched_cost_bp']:.2f}, C_r={base['expected_rolling_cost_bp']:.2f}, "
                    f"\\sigma={base['roll_risk_proxy_bp']:.2f}, \\lambda={base['risk_aversion_multiplier']:.2f}$"
                ),
                derivation_steps=("Compute risk-adjusted rolling cost.", "Compare against matched cost.",),
                assumptions=("Rolling hedge incurs volatility-scaled risk penalty.",),
                interpretation="Preferred hedge minimizes expected risk-adjusted cost.",
                common_misunderstandings=("Comparing expected costs without risk-adjustment term.",),
                result=f"drift={base['expected_roll_drift_bp']:.2f} | RA roll={base['risk_adjusted_rolling_cost_bp']:.2f} | preferred={base['preferred_hedge']}",
            ),
        ]
    )

    # --- Pedagogical footer ---
    render_page_footer(5)
    nominal_diff_bp = (huf - usd) * 10_000.0
    gross_pickup_bp = translate_spread_bp(nominal_diff_bp, curve_cf)
    round_trip = spread_translation_round_trip_bp(nominal_diff_bp, curve_cf, tolerance_bp=1e-6)

    rows: list[dict[str, float | str]] = []
    for hedge_cost_bp in [20.0, 35.0, 50.0]:
        roll_payload = roll_cost_and_risk_proxy_bp(
            current_roll_cost_bp=hedge_cost_bp,
            expected_roll_cost_bp=hedge_cost_bp + 5.0,
            roll_volatility_bp=18.0,
            horizon_years=1.0,
        )
        hedge_choice_payload = matched_vs_rolling_hedge_economics_bp(
            matched_hedge_cost_bp=hedge_cost_bp + 12.0,
            expected_rolling_cost_bp=hedge_cost_bp,
            roll_risk_proxy_bp=float(roll_payload["roll_risk_proxy_bp"]),
            risk_aversion_multiplier=0.6,
        )
        decomposition = hedged_pickup_decomposition_bp(
            gross_spread_pickup_bp=gross_pickup_bp,
            hedge_cost_bp=hedge_cost_bp,
            basis_drag_bp=abs(basis),
            extra_friction_bp=8.0,
        )
        rows.append(
            {
                "hedge_cost": hedge_cost_bp,
                "pickup": decomposition["net_hedged_pickup_bp"],
                **decomposition,
                **roll_payload,
                **hedge_choice_payload,
            }
        )

    base = next(row for row in rows if row["hedge_cost"] == 35.0)
    return {
        "spot": spot,
        "fwd_1y": fwd_1y,
        "basis_bps": basis,
        "simple_cf": simple_cf,
        "curve_cf": curve_cf,
        "nominal_diff_bp": nominal_diff_bp,
        "gross_pickup_bp": gross_pickup_bp,
        "round_trip": round_trip,
        "rows": rows,
        "base": base,
    }


def render_page() -> None:
    import streamlit as st
    from streamlit_calc_helpers import (
        CalculationWindow,
        SignConventionContext,
        render_calculation_windows,
        render_shared_sign_convention,
    )
    from ui_shell import LEARNING_PATH, learning_hint, render_global_shell

    st.set_page_config(page_title="6. Hedged pickup and hedge choice", page_icon="📘", layout="wide")
    render_global_shell()
    st.session_state.suggested_page = LEARNING_PATH[5]


    st.title("6. Hedged pickup and hedge choice")
    render_pedagogical_scaffold(
        st,
        page_number=6,
        learning_path=LEARNING_PATH,
        quantitative_outputs=(
            "Simple and curve-aware conversion factors",
            "Gross spread translated into hedged quote space",
            "Net hedged pickup across hedge-cost assumptions",
            "Matched-vs-rolling preferred hedge",
        ),
        derivation_items=(
            ("Conversion-factor construction", "Compute F/S simple CF, then annuity-weighted curve-aware CF."),
            ("Pickup decomposition", "Net pickup = gross pickup - hedge cost - basis drag - extra frictions."),
            ("Hedge-choice rule", "Compare matched cost with risk-adjusted rolling cost: C_r + λ·σ_roll."),
        ),
    )

    payload = _build_payload(st.session_state)
    base = payload["base"]

    a, b, c, d = st.columns(4)
    a.metric("Simple CF (F/S)", f"{payload['simple_cf']:.6f}")
    b.metric("Curve-aware CF", f"{payload['curve_cf']:.6f}")
    c.metric("Converted gross", f"{payload['gross_pickup_bp']:.2f} bps")
    d.metric("Net pickup", f"{base['pickup']:.2f} bps")
    st.caption(f"Preferred hedge method (base case): **{str(base['preferred_hedge']).title()}**")

    st.line_chart(
        {
            "hedge_cost": [row["hedge_cost"] for row in payload["rows"]],
            "pickup": [row["pickup"] for row in payload["rows"]],
            "benefit_of_rolling": [row["benefit_of_rolling_bp"] for row in payload["rows"]],
        },
        x="hedge_cost",
    )
    st.dataframe(payload["rows"], use_container_width=True)
    learning_hint("Rolling hedges can lose after volatility-scaled roll-risk penalties.")

    round_trip = payload["round_trip"]
    sign_context = SignConventionContext(
        quote_convention="HUF per USD",
        perspective="Investor comparing hedged pickup and hedge implementation (matched vs rolling) for HUF/USD carry.",
        positive_interpretation="Positive pickup/benefit means attractive net economics under the selected hedge method.",
        negative_interpretation="Negative pickup/benefit means hedge costs and frictions dominate carry.",
    )
    render_shared_sign_convention(sign_context)
    render_calculation_windows(
        [
            CalculationWindow(
                title="Simple conversion factor",
                meaning="Direct quote-space conversion ratio between forward and spot.",
                significance="Provides a fast translation between HUF-bp and USD-bp scales.",
                formula=r"CF_{simple}=F/S",
                methodology="Use tenor-matched forward divided by spot.",
                inputs="1Y forward and spot, both in HUF per USD.",
                substituted_values=f"$F={payload['fwd_1y']:.4f}, S={payload['spot']:.4f}$",
                derivation_steps=("Take forward/spot ratio.",),
                assumptions=("Single-tenor approximation.",),
                interpretation="Higher CF maps a given HUF spread into a larger USD spread.",
                common_misunderstandings=("Assuming spot alone determines conversion.",),
                result=f"{payload['simple_cf']:.6f}",
            ),
            CalculationWindow(
                title="Curve-aware conversion factor",
                meaning="Weighted conversion factor across tenor buckets.",
                significance="Improves translation accuracy when carry lives across the curve.",
                formula=r"CF_{curve}=\sum_i w_i(F_i/S)",
                methodology="Apply annuity-style weights to tenor-specific forward ratios.",
                inputs="Spot plus tenor ladder forwards and normalized weights.",
                substituted_values=f"$\\sum_i w_i=1, S={payload['spot']:.4f}$",
                derivation_steps=("Compute F_i/S for each tenor.", "Weight by w_i.", "Sum across tenors.",),
                assumptions=("Weights represent relevant exposure profile.",),
                interpretation="Divergence from simple CF indicates curve-shape effects.",
                common_misunderstandings=("Treating conversion as a constant across maturities.",),
                result=f"{payload['curve_cf']:.6f}",
            ),
            CalculationWindow(
                title="Spread translation round-trip",
                meaning="Consistency check for conversion-factor mapping.",
                significance="Verifies numerical stability and unit discipline of translations.",
                formula=r"\text{HUF bp}\to\text{USD bp}\to\text{HUF bp}",
                methodology="Translate out and back; residual should be near zero.",
                inputs="Input HUF spread, computed USD translation, and reverse translation.",
                substituted_values=(
                    f"${round_trip['huf_bp_in']:.2f}\\to{round_trip['usd_bp_translated']:.2f}"
                    f"\\to{round_trip['huf_bp_round_trip']:.2f}$"
                ),
                derivation_steps=("Translate HUF to USD using CF.", "Translate USD back to HUF.", "Measure residual.",),
                assumptions=("Same CF convention used both directions.",),
                interpretation="Residual near zero validates conversion consistency.",
                common_misunderstandings=("Comparing translated values without checking round-trip error.",),
                result=(
                    f"residual={round_trip['round_trip_residual_bp']:.6f} bp"
                    f" | tol={round_trip['tolerance_bp']:.6f}"
                    f" | pass={round_trip['round_trip_within_tolerance']}"
                ),
            ),
            CalculationWindow(
                title="Pickup decomposition",
                meaning="Net hedged pickup decomposition from gross carry to executable edge.",
                significance="Shows where nominal differential is consumed by costs.",
                formula=r"\text{Net}=\text{Gross}-\text{Hedge}-\text{Basis}-\text{Extra}",
                methodology="Subtract each implementation drag from gross pickup.",
                inputs="Gross pickup, hedge cost, basis drag, extra friction (bps).",
                substituted_values=(
                    f"${base['gross_pickup_bp']:.2f}-{base['hedge_cost_bp']:.2f}"
                    f"-{base['basis_drag_bp']:.2f}-{base['extra_friction_bp']:.2f}$"
                ),
                derivation_steps=("Start from gross pickup.", "Subtract hedge and basis drag.", "Subtract residual frictions.",),
                assumptions=("Cost terms are additive at the chosen horizon.",),
                interpretation="Positive result indicates remaining hedged carry after costs.",
                common_misunderstandings=("Equating nominal carry with realizable pickup.",),
                result=f"{base['net_hedged_pickup_bp']:.2f} bps",
            ),
            CalculationWindow(
                title="Matched vs rolling hedge",
                meaning="Risk-adjusted comparison of hedge implementations.",
                significance="Determines preferred hedge structure beyond expected carry.",
                formula=r"\text{RA roll}=C_{roll}+\lambda\cdot \sigma_{roll}",
                methodology="Penalize rolling strategy by volatility-scaled risk term.",
                inputs="Matched cost, expected rolling cost, roll risk proxy, risk aversion.",
                substituted_values=(
                    f"$C_m={base['matched_cost_bp']:.2f}, C_r={base['expected_rolling_cost_bp']:.2f},"
                    f" \\sigma={base['roll_risk_proxy_bp']:.2f},"
                    f" \\lambda={base['risk_aversion_multiplier']:.2f}$"
                ),
                derivation_steps=("Compute RA rolling cost.", "Compare against matched cost.", "Select cheaper risk-adjusted route.",),
                assumptions=("Risk aversion multiplier is user-appropriate.",),
                interpretation="Lower risk-adjusted cost determines preferred hedge.",
                common_misunderstandings=("Choosing rolling solely on expected drift without risk penalty.",),
                result=(
                    f"drift={base['expected_roll_drift_bp']:.2f}"
                    f" | RA roll={base['risk_adjusted_rolling_cost_bp']:.2f}"
                    f" | preferred={base['preferred_hedge']}"
                ),
            ),
        ],
        sign_convention=sign_context,
    )


if __name__ == "__main__":
    render_page()
else:
    render_page()
