...
def render_page() -> None:
    ...
    render_calculation_windows(
        [
            CalculationWindow(
                ...
                meaning="Implied USD funding rate from spot-forward parity without basis adjustment.",
                significance="Benchmark synthetic funding cost before basis distortions.",
                formula=r"r=\frac{\frac{1+r_{HUF}T}{F/S}-1}{T}",
                methodology="Invert parity using HUF leg accrual and observed forward/spot ratio.",
                inputs=f"Spot S={spot:.4f}, forward F={forward:.4f}, HUF rate={huf_rate:.4%}, tenor T=1Y.",
                substituted_values=f"$S={spot:.4f}, F={forward:.4f}, r_{{HUF}}={huf_rate:.4%}$",
                derivation_steps=("Compute F/S.", "Apply inversion formula.", "Annualize for 1Y tenor.",),
                assumptions=("Positive rate = higher funding cost.",),
                interpretation="Higher implied rate means more expensive synthetic USD borrowing.",
                common_misunderstandings=("Ignoring quote convention can invert sign interpretation.",),
                result=f"{out['synthetic_usd_rate_no_basis']:.4%}",
            ),
            CalculationWindow(
                title="Synthetic USD (with basis)",
                meaning="Implied USD funding rate after incorporating cross-currency basis on the HUF leg.",
                significance="Shows all-in synthetic USD funding cost under market basis conditions.",
                formula=r"r=\frac{\frac{1+(r_{HUF}+b)T}{F/S}-1}{T}",
                methodology="Shift HUF rate by basis b and recompute parity inversion.",
                inputs=f"Same inputs as no-basis case plus basis b={basis:.4%}.",
                substituted_values=f"$b={basis:.4%}$",
                derivation_steps=("Shift HUF rate by b.", "Recompute implied USD rate.", "Compare to no-basis rate.",),
                assumptions=("Positive basis increases HUF coupon.",),
                interpretation="Positive basis increases synthetic USD funding cost in this setup.",
                common_misunderstandings=("Treating basis as unrelated to forward-implied funding.",),
                result=f"{out['synthetic_usd_rate_with_basis']:.4%}",
            ),
            CalculationWindow(
                title="Basis drag",
                meaning="Incremental synthetic funding penalty from adding basis.",
                significance="Expresses basis impact directly in basis points.",
                formula=r"(r_{with}-r_{no})\times 10{,}000",
                methodology="Subtract no-basis implied rate from with-basis implied rate.",
                inputs="Implied USD rates with and without basis.",
                substituted_values=f"$({out['synthetic_usd_rate_with_basis']:.6f}-{out['synthetic_usd_rate_no_basis']:.6f})\\times10,000$",
                derivation_steps=("Take rate difference.", "Multiply by 10,000 to convert to bps.",),
                assumptions=("Positive drag means worse synthetic funding.",),
                interpretation="Positive drag indicates basis worsens synthetic USD economics.",
                common_misunderstandings=("Confusing drag with absolute USD rate level.",),
    ...
    render_calculation_windows([
        CalculationWindow(
            title="Synthetic USD (no basis)",
            meaning="Implied USD funding rate from spot-forward parity excluding basis.",
            significance="This is the benchmark synthetic USD cost before basis distortions.",
            formula=r"r=\frac{\frac{1+r_{HUF}T}{F/S}-1}{T}",
            methodology="Invert CIP to recover the USD-equivalent funding rate.",
            inputs=f"Spot/forward in HUF per USD, r_HUF={huf_rate:.4%}, tenor=1Y.",
            substituted_values=f"$S={spot:.4f}, F={forward:.4f}, r_{{HUF}}={huf_rate:.4%}$",
        ...
        CalculationWindow(
            title="Synthetic USD (with basis)",
            meaning="Implied USD funding rate after adding cross-currency basis to HUF leg.",
            significance="Shows the all-in synthetic USD funding cost faced in market quotes.",
            formula=r"r=\frac{\frac{1+(r_{HUF}+b)T}{F/S}-1}{T}",
            methodology="Basis shifts the HUF coupon used in parity reconstruction.",
            inputs=f"Same as above plus basis b={basis:.4%}.",
            substituted_values=f"$b={basis:.4%}$",
        ...
        CalculationWindow(
            title="Basis drag",
            meaning="Incremental funding penalty from including basis.",
            significance="Quantifies basis impact directly in basis points.",
            formula=r"(r_{with}-r_{no})\times 10{,}000",
            methodology="Take the difference between with/without-basis implied rates.",
            inputs="Implied USD rates from previous two windows.",
            substituted_values=f"$({out['synthetic_usd_rate_with_basis']:.6f}-{out['synthetic_usd_rate_no_basis']:.6f})\\times10,000$",
